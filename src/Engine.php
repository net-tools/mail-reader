...
class Engine
{
	...
	*/
	static function decodeBody($body, $encoding)
	{
		...
		throw new Error("Content-Transfer-Encoding '$encoding' not supported");
	}
	...
	static function decodeCharset($body, $charset)
	{
		...
		{	
			...
			if ( $s === FALSE )
				throw new Error("Decoding from charset '$charset' to UTF-8 error.");
			else
				return $s;
		}
		else
	 ...
	 * @return \Nettools\Mailing\MailBuilder\Content Returns the part decoded to a Content instance 
	 * @throws Error	 
	 */
	static function decodeContent($body, $headers, $contentType)
	{
		...
		if ( !$contentDisposition )
		{
			...
			if ( in_array($contentDisposition, ['attachment', 'inline']) )
			{
				...
				throw new Error("Body with Content-Disposition '$contentDisposition' not supported.");
	}
...
/*				if (!$decodedBody )
			...
			case 'multipart/related' : 
				...
				if ( !$boundary )
					throw new Error("Error when extracting boundary from '$contentType'.");
				...
				if ( count($parts) < 3 )
					throw new Error("Decoding of '$contentType' is impossible because of the unsupported parts number (1).");
				...
				foreach ( $parts as $k=>$part )
				{
					...
					// decode this part
					$partObject = Reader::fromString($part)->email;
					/*if ( !$partObject )
						return NULL;*/
				...
				if ( count($parts) < 2 )
					throw new Error("Decoding of '$contentType' is impossible because of the unsupported parts number (2).");
	...
	*/
	static function fromString($data)
	{
		...
		if ( !$headers )
			throw new Error('Headers cannot be extracted.');
		...
		if ( !$ct ) 
			throw new Error('Header \'Content-Type\' missing.');
